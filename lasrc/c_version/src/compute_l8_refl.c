...
)
{
    ...
    float *taero1 = NULL;  /* aerosol values for each pixel, nlines x nsamps */
    ...
    int16 *aerob7 = NULL; /* L8 atmospherically corrected band 7 data
                             (TOA refl), nlines x nsamps */
    int16 *aod550 = NULL;      /* avg NDWI [RATIO_NBLAT x RATIO_NBLON] */
    ...
    int nelement  = sizeof(taero)/sizeof(taero[0]);
    aod550 = (int16*)malloc(nelement*sizeof(int16));
    for(i = 0; i< nelement; ++i)
    	aod550[i] = int16(taero[i]*1000.0 + 0.5);

    /* Write the aerosol QA band */
    printf ("  Aerosol Band %d: %s\n", SR_L8_AEROSOL+1,
            sr_output->[SR_L8_AEROSOL].file_name);
    if (put_output_lines (sr_output, aod550, SR_L8_AEROSOL, 0, nlines,
        sizeof (int16)) != SUCCESS)
    {
